...
The shipped CLI continues to reject `--host 0.0.0.0`. Launch-token authentication does not imply supported network deployment, TLS, forwarding-header interpretation, or proxy configuration.
...
**Add launch-token logout, TLS-proxy, and forwarding-header configuration.** The loopback token application does not require them. Password mode owns its separate `/auth/logout` route and HTTPS deployment posture in the [Web password authentication decision](2026-09-05-web-password-authentication.md); neither mode accepts proxy identity or forwarding headers. Browser site-data controls revoke one token-mode browser session; deleting the credential record and restarting the process revokes all sessions.
...
The persistent secret makes token-mode cookies survive restarts but gives a stolen cookie up to the configured absolute lifetime. Deleting the record and restarting the process is the global revocation mechanism; the active Connection intentionally avoids credential-provider work on each request. Omitting `Secure` preserves loopback HTTP and permits plaintext transmission if an operator makes the same cookie authority reachable over an unencrypted network. The startup URL contains a process credential and must be treated as sensitive output; runtime diagnostics do not repeat it.

This decision remains the active authority for launch-token mode when `passwordLogin` is absent. [Web password authentication](2026-09-05-web-password-authentication.md) owns enabled password mode, which neither accepts a launch token nor inherits this note's URL exchange. Together, the authentication decisions partially supersede the authentication deferral and unauthenticated non-loopback consequences in the [browser trust note](2026-07-28-api-browser-trust-boundary.md). That note remains active authority for media-type, Host, Origin, Fetch-Metadata, and configured-authority validation. No active Agent Note is archived: the overlap is partial and all three security rules retain future decision value.
