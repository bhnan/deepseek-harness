...
Host 测试覆盖严格 base64 校验、空文件、解码后大小限制、文件名／路径安全、私有上传目录、符号链接拒绝、确定性冲突命名、RPC 分发、未知 Workspace 和业务错误映射。Client 测试覆盖原生选择器与混合拖放分流、仅通用文件拖放、粘贴文件上传、包含带空格路径的引用插入和 API 字节编码。独立工作树会在交回此分支前运行 Host 与 Client 聚焦套件，以及组装后的 GUI／Web 回归。
