...
`host.readFile` 为工作区文件树的应用内预览读取一个常规文件，服务端有界：文本预览只携带 UTF-8 头部（256 KiB，`truncated` 标注截断），按扩展名识别的图片整幅 base64 编码、不超过 8 MiB 并附 `mime`，其余一切——含 NUL 的内容、超限图片——应答 `kind: 'binary'` 且不带内容。非完全限定的路径（与 browse 列举同一道栅栏），或指向不可读、非常规目标的路径，以 `file-unreadable` 失败。该方法在 [`dsh-client-connection`](../../client/connection/README.md) 中与配置面并列钉在回环：它能读取任意宿主文件系统内容，因此已声明的 `trustedHosts` 权威永远到不了它。
