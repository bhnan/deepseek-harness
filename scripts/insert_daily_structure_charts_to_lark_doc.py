#!/usr/bin/env python3
"""Insert daily-structure charts into a Lark report."""
...
MANIFEST_PATH = Path("lab/backtests/sw_sector_daily_structure_analysis/structure_charts/chart_manifest.json")
RESULT_PATH = Path("lab/backtests/sw_sector_daily_structure_analysis/structure_charts/lark_insert_results.json")
...
def run_lark(args: list[str], *, retries: int = 2) -> dict:
    ...
    for attempt in range(retries + 1):
        ...
        if proc.returncode == 0:
            result = extract_json(last_output)
...
def fetch_heading_map(doc: str) -> dict[str, str]:
    ...
    return {title: block_id for block_id, title in re.findall(r'<h2 id="([^"]+)">([^<]+)</h2>', content)}
...
def main() -> int:
    ...
    for item in manifest:
        ...
        time.sleep(0.8)
    print(f"inserted_or_skipped {len({row['code'] for row in results})} charts")
    return 0
