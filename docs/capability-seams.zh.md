...
flowchart LR
  ...
  svc_workspaceFilesController["ctx.workspaceFilesController<br/>Host Workspace file-tree Remote controller"]
  ...
  pkg_api_workspace_controller --> svc_workspaceFilesController
  ...
  svc_workspaceFilesController --> pkg_client_ui_file_tree
...
| `ctx.workspaceFilesController` | `core` | [`api-workspace-controller`](../packages/api/workspace-controller) | - | [`client-ui-file-tree`](../packages/client/ui-file-tree) | - | 通过生成的 `workspaceFiles` Remote namespace 负责有界、只读的 Host 目录列举和常规文件预览投递。 |
